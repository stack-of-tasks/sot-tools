    ...
    */
  {
    ...
    const std::string docstring1 =
     ...
     addCommand("setParams",
    	       dynamicgraph::command::makeCommandVoid3(*this,
   ...
   void KinematicPlanner::blending() {
	 ...
	 if (t<nTimeSteps/10){
	   ...
	   if(cG!=0){
		...
		npSource(0)*wNonPeriodic.col(cG))/2;
   ...
   void KinematicPlanner::goalAdaption(dynamicgraph::Vector& goals, const std::string &dir){
	 ...
	 for (int cs1=0;cs1<nSources1;cs1++) {
	   wPeriodic1.at(cs1).col(j) =
	     wwn.segment(j*(nJoints*(nSources1+nSources2+2))+cs1*nJoints, nJoints);
	 }
	 for (int cs2=0;cs2<nSources2;cs2++) {
	   wPeriodic2.at(cs2).col(j) =
	     wwn.segment(j*(nJoints*(nSources1+nSources2+2))+nJoints*nSources1+cs2*nJoints
			 ,nJoints);
	 }
	 wNonPeriodic.col(j) =
	   wwn.segment(j*(nJoints*(nSources1+nSources2+2))+nJoints*(nSources1+nSources2)
		       ,nJoints);
	 
	 mJointAngle.col(j) = 
	   wwn.segment(j*(nJoints*(nSources1+nSources2+2))+nJoints*(nSources1+nSources2+1)
		       ,nJoints);
       }
     }//local scope01
	 ...
	 M(k) = 
       ...
       //first two steps
       for (int cs1=0;cs1<nSources1;cs1++) {
	 wPeriodic1.at(cs1).col(2) =
	   wwn.segment(cs1*nJoints, nJoints);
       }
       for (int cs2=0;cs2<nSources2;cs2++) {
