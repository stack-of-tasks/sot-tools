...
*/
{
  ...
  const std::string docstring1 =
      ...
      "        - STRING: Path of the directory from where parameters can be "
      "loaded \n"
      "\n";
  addCommand("setParams", dynamicgraph::command::makeCommandVoid3(
                              *this, &KinematicPlanner::setParams, docstring1));
...
void KinematicPlanner::setParams(const double& _distanceToDrawer,
  ...
  for (int i = 0; i < nSources1; i++)
    wPeriodic1.at(i).resize(nJoints, nGaitCycles);

  wPeriodic2.resize(nSources2);
  for (int i = 0; i < nSources2; i++)
    wPeriodic2.at(i).resize(nJoints, nGaitCycles);

  // Mean joint angles
  mJointAngle.resize(nJoints, nGaitCycles);

  loadSourceDelays(dir);
  dynamicgraph::Vector goalsNew =
      createSubGoals(_distanceToDrawer, _objectPositionInDrawer);
...
void KinematicPlanner::loadTrainingParams(const std::string& dir,
                                          dynamicgraph::Matrix& q,
                                          dynamicgraph::Matrix& beta3,
                                          Eigen::ArrayXd& mwwn, double& sigma2,
                                          int& N, int& K) {
...
void KinematicPlanner::delaySources() {
  ...
  for (int cur_src = 0; cur_src < nSources1; cur_src++) {
    pSourceInFreq.matrix().col(cur_src) =
        fft.fwd(pSource1.matrix().col(cur_src));
  }
  ...
  for (int cJ = 0; cJ < nJoints; cJ++) {
    ...
    for (int cur_src = 0; cur_src < nSources1; cur_src++) {
      ...
      const Eigen::ArrayXcd delayedFT =
          pSourceInFreq.col(cur_src) * (V * pDelay1(cJ, cur_src)).exp();
      //(V*pDelay1(cT*nJoints+cJ,
      // cur_src)).unaryExpr<std::complex(*)(std::complex)>(&std::exp);
      // FFT Inv
  ...
  for (int cur_src = 0; cur_src < nSources2; cur_src++) {
    pSourceInFreq.matrix().col(cur_src) =
        fft.fwd(pSource2.matrix().col(cur_src));
  }
  ...
  for (int cJ = 0; cJ < nJoints; cJ++) {
    ...
    for (int cur_src = 0; cur_src < nSources2; cur_src++) {
      // DelayedFt = Source(w) * exp^(V*pDelay1(joint, source))
      const Eigen::ArrayXcd delayedFT =
          pSourceInFreq.col(cur_src) * (V * pDelay2(cJ, cur_src)).exp();
      //(V*pDelay2(cT*nJoints+cJ,
      // cur_src)).unaryExpr<std::complex(*)(std::complex)>(&std::exp);
      // FFT Inv
      pSourceDelayedJoint.matrix().col(cur_src) = fft.inv(delayedFT.matrix());
    }
...
void KinematicPlanner::blending() {
  ...
  for (int cG = 0; cG < nGaitCycles; cG++) {  // nGaitCycles = 4
    ...
    for (int t = 0; t < nTimeSteps; t++) {  // nTimeSteps = 160
      ...
      if (t < nTimeSteps / 10) {
        ...
        if (cG != 0) {
          ...
          Eigen::ArrayXd wnpbeg =
              (npSource(nTimeSteps - 1) * wNonPeriodic.col(cG - 1) +
               npSource(0) * wNonPeriodic.col(cG)) /
              2;
          // interpolation weight
          double ww = (std::sin(t / (nTimeSteps / 10) * pi / 2) + 1) / 2;
          wCurrent =
              (2 * ww - 1) * wNonPeriodic.col(cG) + (1 - (2 * ww - 1)) * wnpbeg;
          mJointAngleCurrent =
              ww * mJointAngle.col(cG) + (1 - ww) * mJointAngle.col(cG - 1);

          for (int cSrc1 = 0; cSrc1 < nSources1; cSrc1++) {
            Eigen::ArrayXd intWeightCSrc =
                ww * wPeriodic1.at(cSrc1).col(cG) +
                (1 - ww) * wPeriodic1.at(cSrc1).col(cG - 1);
            currentWeight1.push_back(intWeightCSrc);
          }

          for (int cSrc2 = 0; cSrc2 < nSources2; cSrc2++) {
            Eigen::ArrayXd intWeightCSrc =
                ww * wPeriodic2.at(cSrc2).col(cG) +
                (1 - ww) * wPeriodic2.at(cSrc2).col(cG - 1);
        ...
        if (cG != nGaitCycles - 1) {
          ...
          Eigen::ArrayXd wnpEnd =
              -(npSource(nTimeSteps - 1) * wNonPeriodic.col(cG) +
                npSource(0) * wNonPeriodic.col(cG + 1)) /
              2;
          double ww =
              (std::sin((t - nTimeSteps) / (nTimeSteps / 10) * pi / 2) + 1) / 2;
          wCurrent = (1 - 2 * ww) * wNonPeriodic.col(cG) + 2 * ww * wnpEnd;
          mJointAngleCurrent =
              (1 - ww) * mJointAngle.col(cG) + ww * mJointAngle.col(cG + 1);

          for (int cSrc1 = 0; cSrc1 < nSources1; cSrc1++) {
            Eigen::ArrayXd intWeightCSrc =
                (1 - ww) * wPeriodic1.at(cSrc1).col(cG) +
                ww * wPeriodic1.at(cSrc1).col(cG + 1);
          ...
          for (int cSrc2 = 0; cSrc2 < nSources2; cSrc2++) {
            Eigen::ArrayXd intWeightCSrc =
                (1 - ww) * wPeriodic2.at(cSrc2).col(cG) +
                ww * wPeriodic2.at(cSrc2).col(cG + 1);
            currentWeight2.push_back(intWeightCSrc);
          }
        }  //(cG!=nGaitCycles-1)
      }  //(t>nTimeSteps*9/10)
      ...
      for (int cJ = 0; cJ < nJoints; cJ++) {
        if (specialInterpolatedWeight) {
          ...
          for (int cSrc1 = 0; cSrc1 < nSources1; cSrc1++)
            allJointsOneGaitCycle(t, cJ) +=
                pSourceDelayed1.at(cJ)(t, cSrc1) * wPeriodic1.at(cSrc1)(cJ, cG);
          for (int cSrc2 = 0; cSrc2 < nSources2; cSrc2++)
            allJointsOneGaitCycle(t, cJ) +=
                pSourceDelayed2.at(cJ)(t, cSrc2) * wPeriodic2.at(cSrc2)(cJ, cG);
    ...
    allJointsAllGaitCycle.middleRows(cG * nTimeSteps, nTimeSteps) =
        allJointsOneGaitCycle;
    allJointsOneGaitCycle.setZero();
  ...
  // Time Stamps
  allJointsAllGaitCycleNew.col(0) = Eigen::ArrayXd::LinSpaced(
      nTimeSteps * nGaitCycles, 1, nTimeSteps * nGaitCycles);
  ...
  allJointsAllGaitCycleNew.row(nTimeSteps * nGaitCycles - 1).segment<18>(22) =
      allJointsAllGaitCycleNew.row(nTimeSteps * nGaitCycles - 2)
          .segment<18>(22);
...
void KinematicPlanner::bSplineInterpolate(Eigen::ArrayXXd& tr, int factor) {
  ...
  for (int cJ = 0; cJ < nJoints; cJ++) {
    Eigen::Spline<double, splDim> spl =
        Eigen::SplineFitting<Eigen::Spline<double, splDim> >::Interpolate(
            trCopy.col(cJ).transpose(), splDeg, x_vec);
    for (int t = 0; t < nTimeSteps * nGaitCycles * factor; t++)
      tr(t, cJ) = spl(x_vec_new(t))(0);
  }
  return;
}

void KinematicPlanner::goalAdaption(dynamicgraph::Vector& goals,
                                    const std::string& dir) {
  ////////////////////////////////////////////////////////////////////
  ...
  {  // local scope01
    ...
    for (int k = 0; k < K; k++) {
      ...
      Eigen::VectorXd _expdiag =
          (-(((X * X.transpose()).diagonal().matrix()) / sigma2)).exp();
      Eigen::MatrixXd Wkern = _expdiag.asDiagonal();
      M(k) = (((X.transpose() * Wkern * X) * X.transpose() * Wkern).inverse() *
              beta3.col(k))(q.cols());
    }
    ...
    for (int j = 0; j < 2; j++) {
      for (int cs1 = 0; cs1 < nSources1; cs1++) {
        wPeriodic1.at(cs1).col(j) = wwn.segment(
            j * (nJoints * (nSources1 + nSources2 + 2)) + cs1 * nJoints,
            nJoints);
      }
      for (int cs2 = 0; cs2 < nSources2; cs2++) {
        wPeriodic2.at(cs2).col(j) =
            wwn.segment(j * (nJoints * (nSources1 + nSources2 + 2)) +
                            nJoints * nSources1 + cs2 * nJoints,
                        nJoints);
      }
      wNonPeriodic.col(j) =
          wwn.segment(j * (nJoints * (nSources1 + nSources2 + 2)) +
                          nJoints * (nSources1 + nSources2),
                      nJoints);

      mJointAngle.col(j) =
          wwn.segment(j * (nJoints * (nSources1 + nSources2 + 2)) +
                          nJoints * (nSources1 + nSources2 + 1),
                      nJoints);
    }
  }  // local scope01
  ...
  {  // local scope3
    ...
    for (int cs2 = 0; cs2 < nSources2; cs2++) {
      wPeriodic2.at(cs2).col(2) =
          wwn.segment(nJoints * nSources1 + cs2 * nJoints, nJoints);
    }
    wNonPeriodic.col(2) =
        wwn.segment(nJoints * (nSources1 + nSources2), nJoints);
    mJointAngle.col(2) =
        wwn.segment(nJoints * (nSources1 + nSources2 + 1), nJoints);
...
void KinematicPlanner::savitzkyGolayFilter(
    Eigen::Ref<Eigen::ArrayXXd> allJointTraj, int polyOrder, int frameSize) {
  assert(polyOrder < frameSize);
  assert(frameSize <=
         allJointTraj.rows());  // Length of the inputs is >= frameSize
  ...
  vanderArr.col(1) = Eigen::ArrayXd::LinSpaced(frameSize, -(frameSize - 1) / 2,
                                               (frameSize - 1) / 2);
  for (int i = 2; i < polyOrder + 1; i++)
    vanderArr.col(i) = vanderArr.col(i - 1) * vanderArr.col(1);
  sotDEBUG(15) << "Printing the VanderArr" << std::endl;
  sotDEBUG(15) << vanderArr << std::endl;

  // Cholesky factor and projection
  Eigen::MatrixXd upperT =
      (vanderArr.matrix().transpose() * vanderArr.matrix()).llt().matrixU();
  ...
  for (int cJ = 0; cJ < allJointTraj.cols(); cJ++) {
    // transient on:
    Eigen::VectorXd yit =
        (proj.bottomRows((frameSize + 1) / 2) *
         allJointTraj.matrix().col(cJ).head(frameSize).reverse())
            .reverse();
    ...
    Eigen::VectorXd yss =
        (proj.row((frameSize - 1) / 2) * xb.middleCols(1, xb.cols() - 2))
            .transpose();

    // transient off:
    Eigen::VectorXd yot =
        (proj.topRows((frameSize + 1) / 2) *
         allJointTraj.matrix().col(cJ).tail(frameSize).reverse())
            .reverse();
...
int& KinematicPlanner::runKinematicPlanner(int& dummy, int time) {
  ...
  //    dynamicgraph::Vector goalsNew =
  //    createSubGoals(distanceToDrawer,objectPositionInDrawer); delaySources();
  blending();
  return dummy;
}
