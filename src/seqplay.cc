  ...
  namespace sot
  {
    namespace tools
    {
      ...
      Seqplay::Seqplay (const std::string& name) :
        ...
        zmpSOUT_
        ("Seqplay(" + name + ")::output(vector)::zmp"),
        ...
        rightAnkle_ (), com_ (), time_ (), R0_ (), R0t_ (), R1_ (), R1R0t_ ()
      {
        signalRegistration (postureSOUT_ << leftAnkleSOUT_ << rightAnkleSOUT_
                            ...
                            << forceLeftFootSOUT_ << forceRightFootSOUT_ << zmpSOUT_);
        ...
        comdotSOUT_.setFunction (boost::bind (&Seqplay::computeComdot, this, _1,
                                              _2));
        comddotSOUT_.setFunction (boost::bind (&Seqplay::computeComddot, this, _1,
                                              _2));

        zmpSOUT_.setFunction (boost::bind (&Seqplay::computeZMP, this, _1,
                                              _2));
      ...
      void Seqplay::load (const std::string& filename)
      {
            ...
            (facultativeFound_[6] && posture_.size () != rightAnkleDot_.size ()))
        {
          ...
          if (facultativeFound_[2])
          {
            oss << "  " << comdot_.size () << " lines from " << filename
                << ".comdot"
                << std::endl;
          }
          ...
          if (facultativeFound_[3])
          {
            oss << "  " << comddot_.size () << " lines from " << filename
                << ".comddot"
                << std::endl;
          }
          ...
          if (facultativeFound_[4])
          {
            oss << "  " << zmp_.size () << " lines from " << filename
                << ".zmp"
                << std::endl;
          }
